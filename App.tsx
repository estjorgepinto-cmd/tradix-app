...
function genCandles(basePrice: number, vol: number, count: number, secs: number): Candle[] {
  const out: Candle[] = [];
...
    const initialData = genCandles(asset.basePrice, asset.volatility, tf.count, tf.seconds);
...
        {/* Chart Area */}
        <div className="bg-slate-900 p-2 rounded-xl border border-slate-800 relative overflow-hidden">
...
             <p className="text-sm text-slate-400 text-center">Analiza el gráfico y decide tu entrada.</p>
          </div>
...
             <p className="text-xs text-slate-500 mt-2">Usa este capital para practicar sin riesgo.</p>
          </div>
        </div>
      </div>
    </div>
